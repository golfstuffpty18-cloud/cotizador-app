...
async function upsertOpportunity(op) {
  ...
  if (placeholder.length) {
    const { rows } = await pool.query(
      `UPDATE opportunities SET
         ...
         category_match = $10, recommendation = $11, reasoning = $12, email_uid = $13, category = $14
      ...
      [op.convocatoria, op.title, op.entity, op.entityAddress, op.entityProvince, op.referencePrice,
       ...
       op.reasoning, op.emailUid ?? null, op.category ?? null, placeholder[0].id]
    );
    return rows[0];
  }
  ...
  const { rows } = await pool.query(
    `INSERT INTO opportunities
      (act_number, convocatoria, title, entity, entity_address, entity_province, reference_price, window_info, deadline, items, category_match, recommendation, reasoning, decision, email_uid, category)
    ...
    [op.actNumber, op.convocatoria, op.title, op.entity, op.entityAddress, op.entityProvince, op.referencePrice, op.windowInfo, op.deadline,
     JSON.stringify(op.items || []), op.categoryMatch, op.recommendation, op.reasoning, op.emailUid ?? null, op.category ?? null]
  );
  return rows[0] || null;
}
